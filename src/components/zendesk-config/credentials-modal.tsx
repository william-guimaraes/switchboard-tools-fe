...
import { useConfigureZendeskCredentials, ZendeskCredentialsDTO } from '../../queries'
...
export const CredentialsModal = ({ isOpen, onCancel }: Props) => {
    const configureCredentials = useConfigureZendeskCredentials()

    const handleSubmit = async (data: ZendeskCredentialsDTO) => {
        try {
    ...
    return (
        < Modal
            title="Configure Zendesk Credentials"
            open={isOpen}
            onCancel={onCancel}
            footer={<div></div>}
            destroyOnClose
